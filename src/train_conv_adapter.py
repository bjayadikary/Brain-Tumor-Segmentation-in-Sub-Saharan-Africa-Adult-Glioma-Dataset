...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if cfg.get("ckpt_path_for_finetuning"):
        ...
        # assert if the len in new_layer_name (keys) in pretrained_checkpoint['state_dict'] matches those in new model with adapter state_dict, except the adapter's weights and biases
        assert len(list(pretrained_checkpoint['state_dict'].keys())) == len(list(x for x in model.state_dict().keys() if 'med_adp' not in x and 'dice_loss' not in x))
