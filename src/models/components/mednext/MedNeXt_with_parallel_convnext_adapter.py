...
from .blocks import *
from .prefixed_mednext_block_for_adapter import PrefixedMedNeXtBlock
# from .linear_adapter_block import LinearAdapter

# To make the adapter parallel new custom module is added that takes input both the 'MedNeXtBlock' and 'PrefixedMedNeXtBlock' in parallel and then combines their outputs by adding them together
class ParallelMedNeXtBlock(nn.Module):
    def __init__(self, mednext_block, adapter_block):
        super(ParallelMedNeXtBlock, self).__init__()
        self.mednext_block = mednext_block
        self.adapter_block = adapter_block

    def forward(self, x):
        # Process input through mednext_block and adapter_block in parallel
        mednext_output = self.mednext_block(x)
        adapter_output = self.adapter_block(x)

        # Combine the outputs
        return mednext_output + adapter_output

class MedNeXtWithParallelAdapters(nn.Module):
    ...
    def __init__(self, 
        ...
        dim = '3d',                                # 2d or 3d
        grn = False,
        # adapter_dim_ratio: float = 0.25 # added, 25% of n_channels will be specifed as adapter dimension
    ):
        ...
        self.enc_block_0 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
            ...
            # LinearAdapter(n_channels, int(n_channels * adapter_dim_ratio)) # added
            PrefixedMedNeXtBlock(
                in_channels=n_channels,
                out_channels=n_channels,
                exp_r=exp_r[0],
                kernel_size=enc_kernel_size,
        ...
        self.enc_block_1 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
            ...
            # LinearAdapter(n_channels*2, int(n_channels*2*adapter_dim_ratio)) # added
            PrefixedMedNeXtBlock(
                in_channels=n_channels*2,
                out_channels=n_channels*2,
                exp_r=exp_r[1],
        ...
        self.enc_block_2 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
            ...
            # LinearAdapter(n_channels*4, int(n_channels*4*adapter_dim_ratio)) # added
            PrefixedMedNeXtBlock(
                in_channels=n_channels*4,
                out_channels=n_channels*4,
                exp_r=exp_r[2],
        ...
        self.enc_block_3 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
            ...
            # LinearAdapter(n_channels*8, int(n_channels*8*adapter_dim_ratio)) # added
            PrefixedMedNeXtBlock(
                in_channels=n_channels*8,
                out_channels=n_channels*8,
                exp_r=exp_r[3],
        ...
        self.bottleneck = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
            ...
            # LinearAdapter(n_channels*16, int(n_channels*16*adapter_dim_ratio)) # added
            PrefixedMedNeXtBlock(
                in_channels=n_channels*16,
                out_channels=n_channels*16,
                exp_r=exp_r[4],
        ...
        self.dec_block_3 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
                    in_channels=n_channels*8,
                    out_channels=n_channels*8,
        ...
        self.dec_block_2 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
                    in_channels=n_channels*4,
                    out_channels=n_channels*4,
        ...
        self.dec_block_1 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
                    in_channels=n_channels*2,
                    out_channels=n_channels*2,
        ...
        self.dec_block_0 = ParallelMedNeXtBlock( # changed here from nn.Sequential() to our custom module
            nn.Sequential(*[
                MedNeXtBlock(
                    in_channels=n_channels,
                    out_channels=n_channels,
