...
def train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    log.info(f"Instantiating model <{cfg.model._target_}> with parallel adapters") # cfg.model._target_ specified the Python class to instantiate. In the example, 'train.yaml', this is 'src.model.MNIST'.
    ...
    if cfg.get("ckpt_path_for_finetuning"):
        ...
        # Changing the keys in pretrained_checkpoint to match the keys in model with parallel adapter state dict
        for layer_name in list(pretrained_checkpoint['state_dict'].keys()):
            if 'enc_block' in layer_name or 'dec_block' in layer_name or 'bottleneck' in layer_name:
                # net.enc_block_0.0.conv1.weight -> net.enc_block_0.mednext_block.0.conv1.weight
                # similarly in dec_block and bottlenect block, need to add string 'mednext_block' at index 2 
        ...
        # assert if the total keys in pretrained_checkpoint with modified layer name matches those in model with parallel adapter state_dict, except adapter_block's parameters
        assert len(list(pretrained_checkpoint['state_dict'].keys())) == len(list(x for x in model.state_dict().keys() if 'adapter_block' not in x and 'dice_loss_fn' not in x))
        ...
        missing_keys, unexpected_keys = model.load_state_dict(pretrained_checkpoint['state_dict'], strict=False)

        assert len(missing_keys) == 73, "Some keys in checkpoint do not have matching keys in new model" # 72 keys in model has to be of adapter which is missing in checkpoint + dice_loss_fn which is later added in the model so, total of 73 keys should be missing in the new adapter model
